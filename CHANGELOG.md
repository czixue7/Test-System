...
- **知识库 AI 总结**：覆盖全部题库（含用户自建题库），大题库行级切块分批 AI 总结后合并，内容零丢失；覆盖范围标注（题库数/题目数/知识库篇数）；生成时间 + 可折叠题库清单
- **知识总结弹窗**：新增"重新生成"按钮（准备中/总结中禁用）；进度显示优化（准备中/分批总结 N/M/合并 + 进度条 + 直白文案），合并阶段文案去冗余
- **AI 搜索双通道回答**：知识总结作为参考 + 实时检索题库原文，回答附来源标注；默认搜索源为 combined（知识库+题库）
- **Markdown 渲染**：知识库和总结弹窗适配 Markdown（react-markdown + remark-gfm + rehype-raw），支持 HTML 折叠框
- **对话管理**：多会话切换/删除 + 本地持久化（chatStore version 2 + 数据迁移），右上角图标改为带加号的聊天气泡
- **知识库"帮我记"**：AI 识别转换后的内容按分类自动入库；转换设置（汉字数字→数字、英文小写转大写、去除空格）；被分类文本可打开到新界面查看/修改
- **导入优化**：仅允许 .txt/.md；导入前弹分类选择（可新建分类）；侧边栏空状态改"暂无内容"
- **知识库冲突优先级**：总结时知识库与题库内容/参数冲突，以知识库优先
- **分类管理**：有内容的分类删除需确认；点击分类标签不关闭侧边栏；条目作为折叠标签子条目展示

### 修复

- **8 处弹窗补齐进出对称动效**：DutySchedule 推算排班/删除确认、KnowledgeBase 对话管理/知识总结/导入分类/"帮我记"设置、ManageBanks 删除确认/导出总结（modal-fade-out/modal-pop-out 0.18s + 180ms 延迟卸载）
- **值班表月份错位**：切换日历月份未选日期时，当天值班名单月份不再跟随变化
- **UI 统一**：四个 tab header 高度/字号/居中统一、底部导航图标统一、滚动条占位抖动（scrollbar-gutter: stable）、值班表页面白边、窄屏按钮文字换行
- **弹窗背景骤变/闪烁**：Profile 关于/主题弹窗独立 closing 状态
- **自定义协议（关键）**：`custom-protocol` 加入默认特性，修复 Android APK 白屏（`Failed to request http://localhost:1420/`）
- **indexmap 构建失败**：本地 vendor patch 规避 build script 执行失败（os error 5）

### 变更

- 版本号升级 0.3.9，Android 构建号（versionCode）20260908
- Android 构建仅产出 arm64 APK
- 关于弹窗显示"版本 0.3.9 / 构建 20260908"
...
- **填空题答案判断优化**：
  - 新增 `normalizeAnswer` 答案标准化函数，统一处理不影响正确性的格式差异
  - 全角/半角字符统一（如 `Ａ` → `A`、`１２３` → `123`、`，` → `,`）
  - 单位大小写统一（如 `M` → `m`、`KG` → `kg`）
  - 数字格式统一：去除小数尾零（`36.0` → `36`、`36.50` → `36.5`）
  - 分隔符统一：`/`、`、`、`，`、`|`、`；` 统一为 `/`
  - 补全小数前导零（`.5` → `0.5`）、去除正号前缀（`+36` → `36`）
  - 去除首尾引号和简单括号
  - 去除末尾标点（`。`、`.`、`，` 等）
  - 空格标准化（合并多余空格、去除首尾空格）
  - 数学符号统一：`×` `✕` `✖` → `*`，`÷` → `/`，`−` → `-`
  - 影响 `examStore.ts`、`aiGrading.ts`、`Practice.tsx` 中所有填空题判断逻辑
...
- **沉浸式主题**：
  - 新增布局优先的"沉浸式"主题（我的 > 主题可切换，默认启用）
  - Hero 大卡 + 统计卡片 + 题库封面 + 模式网格 + 浮动毛玻璃导航
  - 默认跟随系统深浅色（跟随系统/浅色/深色三态）
  - 经典主题保留原有布局，两种布局互不影响
- **弹窗毛玻璃增强**：
  - 遮罩背景模糊（blur 12px）+ 弹窗本体半透明玻璃质感
  - 高光边框 + 柔和投影，浅色/深色两套适配
  - 关于、主题、退出确认、恢复进度等弹窗统一生效
  - 关于弹窗信息卡片毛玻璃化，版本与描述合并为一张卡片
- **调试日志美化**：
  - 检查更新/下载/安装日志按级别着色（成功绿/警告黄/错误红/信息蓝）
  - 时间戳与消息拆分展示，毛玻璃日志面板 + 细滚动条
  - 移除下载进度逐条刷屏日志（进度条已实时展示）
- **退出保存恢复进度**：
  - 顺序练题/模拟考试中途退出自动保存进度
  - 再次进入可一键恢复（退出确认弹窗 + 恢复进度弹窗）
  - 交卷后自动清除已保存进度
- **检查更新失败提示优化**：
  - 失败与"已是最新版本"明确区分
  - 失败时按钮显示"检查失败，点击重试"（红色），日志展示具体错误原因
  - 修复 Tauri invoke 抛出字符串时显示笼统"检查更新失败"的问题
...
- **下载进度实时显示**：
  - 修复下载安装包时进度条不显示进度的问题
  - 使用流式下载和 Tauri Channel 实现实时进度推送
  - 后端使用 reqwest 流式下载，前端实时更新进度条
- **更新检测增强**：
  - 除版本号外，新增哈希值比对机制
  - 同一版本的不同构建也能被检测为更新
  - 在关于页面显示构建哈希值
...
- **顺序练题颜色显示修复**：
  - 修复固定判题提交后，错误答案的输入框也会变成绿色的 bug
  - 现在每个空根据实际对错显示绿色（正确）或红色（错误）
- **考试结果页面布局修复**：
  - 移除底部多余的空白遮挡区域
  - 优化内容区域高度计算
...
- **答题详情页面滑动切换**：
  ...
  - 整个页面区域均可触发滑动
- **智谱AI免费模型支持**：
  - 新增 GLM-4-Flash（200并发，推荐批量判题）
  - 新增 GLM-4.7-Flash（1并发，适合单题精判）
  - 新增 GLM-Z1-Flash（30并发，推理模型）
  - 新增 GLM-4-Flash-250414（5并发，新版模型）
  - 所有模型均免费调用
- **判题状态信息框优化**：
  - 信息框内容居中显示
  - 添加动态扫描背景效果
  - 添加加载动画点
  - 判题阶段与实际行为对应（连接中/处理中/生成结果）
...
- **输入框样式统一**：
  ...
  - 选中时显示淡蓝色线框（渐显效果）
  - 失去焦点时线框渐隐
  - 保留答案正确/错误时的颜色变化（绿色/橙色/红色）
- **模型列表更新**：
  - 更新智谱AI模型列表，标注并发限制
  - GLM-4-Flash 标注为"推荐-200并发"
  - 移除已下线的模型选项
