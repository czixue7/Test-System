...
const Home: React.FC = () => {
  ...
  const { banks, addBank } = useQuestionBankStore();
  ...
  useEffect(() => {
    if (showAddModal || showCreateModal) {
  ...
  const handleCreateBank = () => {
    ...
    setCurrentBankId(id);
    localStorage.setItem(STORAGE_KEY_CURRENT_BANK, id);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 safe-header">
      <header className="bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg dark:from-blue-700 dark:to-blue-800">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-24">
          ...
          <div className="bg-white rounded-2xl shadow-md mb-6 border border-gray-100 relative dark:bg-gray-800 dark:border-gray-700">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base font-bold text-gray-800 dark:text-white">{currentBank.name}</h3>
                    {isBuiltInBank(currentBank.id) && <span className="px-2 py-0.5 bg-blue-100 text-blue-600 text-xs rounded-full dark:bg-blue-900 dark:text-blue-300 flex-shrink-0">内置</span>}
                  </div>
            ...
            {isBankListExpanded && banks.length > 1 && (
              <div className="absolute left-0 right-0 top-full mt-1 max-h-60 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-lg z-10 rounded-2xl" onWheel={(e) => e.stopPropagation()}>
                {banks.map((bank) => (
                  <button key={bank.id} onClick={() => handleSelectBank(bank.id)} className={`w-full px-4 py-3 text-left flex items-center justify-between transition-colors ${currentBankId === bank.id ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300' : 'hover:bg-gray-50 text-gray-700 dark:hover:bg-gray-700 dark:text-gray-300'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {currentBankId === bank.id && <svg className="w-5 h-5 text-blue-500 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                    </div>
                  </button>
                ))}
              </div>
        ...
        {!currentBank && banks.length === 0 && (
          <div className="bg-white rounded-2xl shadow-md p-6 mb-6 text-center border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
            ...
            <button onClick={() => setShowAddModal(true)} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors dark:bg-blue-600 dark:hover:bg-blue-700">新建题库</button>
          </div>
        )}
      ...
      {showAddModal && (
        ...
        >
          <div 
            className="bg-white rounded-2xl p-5 w-full max-w-sm dark:bg-gray-800 shadow-2xl transform transition-all duration-300 ease-out"
            style={{
          ...
          >
            ...
            <div className="space-y-3">
              <button onClick={() => { setShowAddModal(false); setShowCreateModal(true); setNewBankName(''); setNewBankDesc(''); }} className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-blue-500 hover:bg-blue-50 transition-all dark:border-gray-700 dark:hover:border-blue-400 dark:hover:bg-blue-900/30">
                <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center dark:bg-blue-900">
                  <svg className="w-5 h-5 text-blue-600 dark:text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                </div>
                <div className="text-left">
                  <div className="font-medium text-gray-800 dark:text-white">新建题库</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">创建一个空白题库</div>
                </div>
              </button>
      ...
      {showCreateModal && (
        ...
        >
          <div 
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-semibold mb-4 dark:text-white">新建题库</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">题库名称 *</label>
                <input type="text" value={newBankName} onChange={e => setNewBankName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-400" placeholder="请输入题库名称" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">题库描述</label>
                <textarea value={newBankDesc} onChange={e => setNewBankDesc(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-400" rows={2} placeholder="请输入题库描述（可选）" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors dark:text-gray-400 dark:hover:bg-gray-700">取消</button>
              <button onClick={handleCreateBank} disabled={!newBankName.trim()} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors disabled:opacity-50 dark:bg-blue-600 dark:hover:bg-blue-700">创建</button>
            </div>
          </div>
        </div>
      )}
