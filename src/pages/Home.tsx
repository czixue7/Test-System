...
const Home: React.FC = () => {
  ...
  const { initTheme, themeStyle } = useThemeStore();
  ...
  const isImmersive = themeStyle === 'immersive';

  // ===== 沉浸式布局（区别于经典布局：Hero 大卡 + 统计横幅 + 模式网格 + 浮动导航） =====
  const immersiveView = (
    <>
      <div className="max-w-lg mx-auto px-4 pb-32" style={{ paddingTop: safeArea.top + 12 }}>
        {/* Hero 大卡 */}
        <div className="im-hero">
          <div className="flex items-start justify-between">
            <div className="min-w-0">
              <div className="im-hero-title">学习</div>
              <div className="im-hero-sub">{getGreeting()}，今天想学点什么？</div>
            </div>
            <button onClick={() => setShowAddModal(true)} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-colors flex-shrink-0">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="im-stat-card">
            <div className="im-stat-value text-blue-600 dark:text-blue-400">{totalQuestions}</div>
            <div className="text-xs text-gray-500 mt-1 dark:text-gray-400">总题数</div>
          </div>
          <div className="im-stat-card">
            <div className="im-stat-value text-red-500 dark:text-red-400">{wrongQuestions.length}</div>
            <div className="text-xs text-gray-500 mt-1 dark:text-gray-400">错题数</div>
          </div>
          <div className="im-stat-card">
            <div className="im-stat-value text-yellow-500 dark:text-yellow-400">{commonQuestions.length}</div>
        ...
        {currentBank && (
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button onClick={() => handleStartPractice('sequential')} disabled={currentBank.questions.length === 0} className="im-mode-card disabled:opacity-50 disabled:cursor-not-allowed text-left">
              <div className="im-mode-icon bg-blue-500">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">顺序练题</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">按顺序练习全部题目</div>
              </div>
            </button>
            <button onClick={handleStartExam} disabled={currentBank.questions.length === 0} className="im-mode-card disabled:opacity-50 disabled:cursor-not-allowed text-left">
              <div className="im-mode-icon bg-orange-500">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">模拟考试</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">限时作答，检验水平</div>
              </div>
            </button>
            <button onClick={() => handleStartPractice('wrong')} disabled={currentBank.questions.length === 0 || wrongQuestions.length === 0} className="im-mode-card disabled:opacity-50 disabled:cursor-not-allowed text-left">
              <div className="im-mode-icon bg-green-500">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">练易错题</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{wrongQuestions.length > 0 ? `${wrongQuestions.length} 道错题待巩固` : '暂无错题'}</div>
              </div>
            </button>
            <button onClick={() => handleStartPractice('view')} disabled={currentBank.questions.length === 0} className="im-mode-card disabled:opacity-50 disabled:cursor-not-allowed text-left">
              <div className="im-mode-icon bg-blue-400">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">看题模式</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">浏览题目与参考答案</div>
              </div>
            </button>
            <button onClick={() => handleStartPractice('common')} disabled={commonQuestions.length === 0} className="im-mode-card col-span-2 disabled:opacity-50 disabled:cursor-not-allowed text-left">
              <div className="im-mode-icon bg-yellow-500">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">收藏</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{commonQuestions.length > 0 ? `${commonQuestions.length} 道收藏题目` : '暂无收藏'}</div>
              </div>
              {commonQuestions.length > 0 && <span className="px-2 py-0.5 text-xs rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300 flex-shrink-0">{commonQuestions.length > 99 ? '99+' : commonQuestions.length}</span>}
            </button>
          </div>
        )}
      </div>

      {/* 底部导航（沉浸式由 CSS 呈现为浮动胶囊） */}
