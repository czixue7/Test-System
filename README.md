...
- **输入框样式优化**：统一答题输入框样式，选中时淡蓝色线框渐显，答案正确/错误时颜色变化
- AI 考试评价：考试结束后生成 AI 总体评价和每题解析
- 主观题智能评分（基于文本相似度算法）
- 答题记录管理
- 错题回顾功能
- 自定义题库导入
- **在线题库下载**：从 GitHub 下载系统题库和用户题库
- **题库管理**：管理已导入题库，支持更新和删除
- **检查更新**：自动检测应用新版本，失败时显示具体原因
- **双主题布局**：经典 / 沉浸式两种布局自由切换，沉浸式带 Hero 大卡、模式网格、浮动毛玻璃导航，默认跟随系统深浅色
...
| AI 推理 | @mlc-ai/web-llm (WebGPU) + API 云端判题 |
...
### Windows 桌面端 v0.3.9
- **安装包**: `答题测试库_0.3.9_x64-setup.exe` (~6MB)
- 支持 Windows 10/11 64位系统
- 下载地址：[Releases](https://github.com/czixue7/Test-System/releases)

### Android 移动端 v0.3.9
- **ARM64 版本**: `Answer_Test_v0.3.9-arm64-release.apk` (~21MB) - 推荐，性能更好
- 支持 Android 8.0+ 系统
- 下载地址：[Releases](https://github.com/czixue7/Test-System/releases)

## 最新更新 (v0.3.9)

- **知识库 AI 总结覆盖全部题库**：含用户自建题库，大题库分批 AI 总结不丢内容，覆盖范围标注（题库数/题目数/知识库篇数）
- **知识总结弹窗升级**："重新生成"按钮（准备/总结中禁用），进度显示优化（准备中/分批 N/M/合并 + 进度条）
- **AI 搜索双通道回答**：知识总结作参考 + 实时检索题库原文，附来源标注；默认搜索源 combined（知识库+题库）
- **Markdown 渲染**：知识库与总结弹窗适配 Markdown（react-markdown + remark-gfm + rehype-raw），综合总结题库清单可折叠
- **对话管理**：多会话切换/删除 + 本地持久化，图标改为带加号聊天气泡
- **知识库"帮我记"**：AI 识别内容按分类入库 + 转换设置（汉字数字→数字、小写转大写、去空格），内容可新界面查看/修改
- **导入优化**：仅 .txt/.md，导入前选分类（可新建），空状态改"暂无内容"
- **知识库冲突优先级**：与题库内容/参数冲突时以知识库优先
- **8 处弹窗进出对称动效**：排班/删除/对话管理/知识总结/导入分类/帮我记设置/题库删除/导出总结全部补齐
- **修复**：值班表月份错位、header 不统一、滚动条抖动、白边、窄屏按钮换行、弹窗背景骤变/闪烁、Android APK 白屏（custom-protocol 默认启用）、indexmap 构建失败（vendor patch）
- **版本**：0.3.9 / 构建号 20260908，Android 仅出 arm64 APK
...
- **更新检测缓存问题修复**：修复构建哈希被截断导致误识别更新的问题，将 GitHub API 调用移到 Rust 后端绕过 WebView 缓存
...
- **填空题答案判断优化**：新增 `normalizeAnswer` 标准化函数，处理全角/半角、大小写、数字尾零、分隔符差异、数学符号（×→*、÷→/）、末尾标点等不影响答案正确性的格式差异
- **沉浸式主题**：新增布局优先的"沉浸式"主题（我的 > 主题可切换），Hero 大卡 + 统计卡片 + 题库封面 + 模式网格 + 浮动毛玻璃导航，默认跟随系统深浅色，经典主题保留原有布局
- **弹窗毛玻璃增强**：遮罩背景模糊 + 弹窗半透明玻璃质感（高光边框 + 柔和投影），关于、主题、退出确认等弹窗统一生效
- **关于弹窗优化**：信息卡片毛玻璃化，版本与描述合并为一张卡片，调试日志分级着色（成功/警告/错误）并显示具体原因
- **退出保存恢复进度**：顺序练题/模拟考试中途退出自动保存进度，再次进入可一键恢复，交卷后自动清除
- **检查更新失败提示优化**：失败与"已是最新版本"明确区分，失败时按钮显示"检查失败，点击重试"并在日志中展示具体错误原因
